...
it('stops being partially refunded once the refunds cover the amount', function (): void {
    ...
    $action->handle($transaction, 60.0);
    $action->handle($transaction, 40.0);
...
it('counts a full refund made in one step', function (): void {
    ...
    resolve(RefundTransactionAction::class)->handle($transaction, 100.0);
...
it('reports the remaining balance without a refund record when the refunds table is missing', function (): void {
    Event::fake([TransactionRefunded::class]);
    (include __DIR__.'/../../database/migrations/create_sisp_refunds_table.php')->down();
    $transaction = refundableHistoryTransaction();

    resolve(RefundTransactionAction::class)->handle($transaction, 30.0);

    expect($transaction->refundedAmount())->toBe(30.0)
        ->and($transaction->isPartiallyRefunded())->toBeTrue()
        ->and(Schema::hasTable('sisp_refunds'))->toBeFalse();

    Event::assertDispatched(
        TransactionRefunded::class,
        fn (TransactionRefunded $event): bool => ! $event->refund instanceof Refund && $event->remainingAmount === 70.0,
    );
});
