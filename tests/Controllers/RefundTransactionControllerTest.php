...
it('keeps the 2.1 controller signature and validates a plain request', function (): void {
    allowRefunds();
    $transaction = refundableTransaction();
    $parameters = new ReflectionMethod(RefundTransactionController::class, '__invoke')->getParameters();

    $request = Request::create('/', 'POST', ['amount' => ['x']]);
    $request->setUserResolver(fn (): RefundRouteUser => new RefundRouteUser());

    $response = rescue(
        fn () => resolve(RefundTransactionController::class)($transaction, $request),
        fn (HttpResponseException $exception) => $exception->getResponse(),
        report: false,
    );

    expect(array_map(fn (ReflectionParameter $parameter): string => (string) $parameter->getType(), $parameters))
        ->toBe([Transaction::class, Request::class])
