...
## 2025-02-18 - [IDOR in Administrative Package Routes]
**Vulnerability:** The `sisp/refund/{transaction}` route relied on the default `auth` middleware, allowing *any* authenticated user to refund any transaction by enumerating IDs. This is a critical Insecure Direct Object Reference (IDOR) because the package cannot inherently distinguish between regular users and admins.
**Learning:** Default middleware like `auth` is insufficient for administrative actions in packages. Packages must provide a flexible authorization mechanism (like a gate or closure callback) that forces the host application to explicitly define *who* is allowed to perform these actions.
**Prevention:** For sensitive package actions, implement a static authorization method (e.g., `Package::auth(Closure $callback)`) and enforce it in controllers. Default to denying access in production if no callback is registered.
