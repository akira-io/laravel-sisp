...
Both the manual query and the scheduled reconciliation below authenticate with `sisp.transaction_status.portal_id` and `sisp.transaction_status.portal_password` (`SISP_PORTAL_ID` / `SISP_PORTAL_PASSWORD`), sent as HTTP Basic credentials. SISP issues these Merchant Portal credentials separately from the POS credentials used to sign payments. A merchant without them has no way to reconcile an indeterminate transaction against SISP; `sisp:expire-pending` exists for exactly that case, to stop those transactions accumulating forever when reconciliation is not configured.
...
Two artisan commands keep old transaction data from accumulating. Neither is scheduled by default; register the ones you want in your application's `routes/console.php`.

#### sisp:expire-pending

Cancels pending SISP transactions that never received a callback, once they are older than a configurable window. Defaults to `30` days (`sisp.expire_pending_after_days` / `SISP_EXPIRE_PENDING_AFTER_DAYS`).

```bash
php artisan sisp:expire-pending
php artisan sisp:expire-pending --older-than=14
...
Removes the 3-D Secure `purchaseRequest` blob from transactions that already reached a terminal status (`completed`, `failed`, `cancelled`, `refunded`), once they are older than a configurable window. Defaults to `90` days (`sisp.prune_request_payloads_after_days` / `SISP_PRUNE_REQUEST_PAYLOADS_AFTER_DAYS`).

```bash
php artisan sisp:prune-request-payloads
...
Register it, for example monthly:
